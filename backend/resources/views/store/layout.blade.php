            ...
            <form action="{{ route('store.search') }}" method="GET" class="hidden md:flex items-center flex-1 max-w-sm">
                <div class="relative w-full group">
                    <input type="text" name="q" value="{{ $query ?? '' }}"
                           ...
                           class="w-full bg-slate-100/50 dark:bg-slate-900/50 border border-slate-200/60 dark:border-slate-800/60 px-4 py-2.5 pl-10 text-xs font-medium focus:border-primary focus:bg-white dark:focus:bg-slate-900 transition-all outline-none rounded-full"
                           autocomplete="off">
